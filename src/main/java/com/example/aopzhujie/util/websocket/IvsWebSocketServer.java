 ...
 */
//@ServerEndpoint("/ivsWebsocket/{userId}")
//public class IvsWebSocketServer {
    //静态变量，用来记录当前在线连接数。应该把它设计成线程安全的。
//    private static int onlineCount = 0;

    //concurrent包的线程安全Set，用来存放每个客户端对应的MyWebSocket对象。若要实现服务端与单一客户端通信的话，可以使用Map来存放，其中Key可以为用户标识
//    private static CopyOnWriteArraySet<IvsWebSocketServer> webSocketSet = new CopyOnWriteArraySet<IvsWebSocketServer>();
...
//        }
    ...
    public void onClose(@PathParam("userId") String userId) {
        ...
        if(IvsWebSocketServer.onlineCount > 0){
...
//        }
   ...
   /* public static synchronized int getOnlineCount() {
    ...
    public static synchronized void addOnlineCount() {
        IvsWebSocketServer.onlineCount++;
    }
...
/*
    *//**
    ...
    public static void sendInfo(String message) throws IOException {
        for (IvsWebSocketServer item : webSocketSet) {
            try {
                item.sendMessage(message);
            } catch (IOException e) {
                continue;
